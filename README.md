...
| EVENT_JOURNAL_DSN | нет | Postgres для журнала членства (см. ниже); пусто — журнал выключен |
...
- asyncpg — доставка журнала членства в Postgres (нужен, только если журнал включён)
...
### Журнал членства (опционально)
Помимо таблицы бот может вести журнал: каждое вступление, выход, удаление и заявку — как они произошли, с тем, кто совершил действие (`from_user`). По нему tg_parser подтверждает, кто кого пригласил, и видит, кто остался. Включается переменной `EVENT_JOURNAL_DSN`.

- Таблица от журнала не зависит: те же строки и те же пропуски, что бы ни случилось с журналом. Журнал видит больше — например, прямые добавления администратором, которые таблица при `LOG_JOINS_WITHOUT_INVITE=false` не показывает.
- Обработчик только кладёт запись в `event_outbox` локальной SQLite; фоновая задача доставляет её в Postgres (`srm.membership_events` базы tg_parser) по порядку и удаляет доставленное. Пока Postgres недоступен, записи ждут и ничего не теряется.
- Повтор апдейта безвреден: ключ — `update_id`, Postgres игнорирует уже известный.
- Запись, которую Postgres отверг по существу (например, неизвестный вид события), откладывается (`parked_at`, `last_error`) и не держит остальные.
- Роль в DSN — `srm_writer`: может только вставлять в одну таблицу, не читает даже её. Схему и роль создаёт миграция tg_parser; пароль и LOGIN выдаются на сервере отдельно.

Контрактные тесты против настоящей схемы (`tests/test_journal_postgres.py`) запускаются, если задать `SRM_TEST_PG_ADMIN_DSN` — DSN администратора базы, где применены миграции tg_parser (подойдёт её тестовый контейнер).
